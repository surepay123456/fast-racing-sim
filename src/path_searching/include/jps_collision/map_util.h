   ...
   public:
    ...
    void setParam(ros::NodeHandle &nh) {
        ...
        */
        ...
        nh.param("cloud_topic", cloud_topic, std::string("/airsim_global_map"));
        ...
        if (use_esdf) {
            ...
            esdf_pub_ =
                nh.advertise<sensor_msgs::PointCloud2>("/global_esdf", 10);
        }
        point_cloud_sub_ = nh.subscribe(cloud_topic, 10,
                                        &MapUtil::GlobalMapBuild, this);
    }
    ...
    void updateESDF3d() {
        ...
        for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
            for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
                fillESDF(
                    [&](int y) { return tmp_buffer1_[toAddress(x, y, z)]; },
                    [&](int y, double val) {
        ...
        for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
            for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
                fillESDF(
                    [&](int x) { return tmp_buffer2_[toAddress(x, y, z)]; },
                    [&](int x, double val) {
